package com.libtop.weitu.utils.viewpagerbitmap;
...
public class ImagePagerActivity extends FragmentActivity implements View.OnClickListener, PhotoViewAttacher.gotoView
{
    ...
    private void addView(List<String> mlist)
    {
        for (int i = 0; i < mlist.size(); i++)
        {
            ...
            {
                Picasso.with(ImagePagerActivity.this).load(imageFile).placeholder(R.drawable.default_error).tag(MultiImageSelectorFragment.TAG).resize(width, width).centerCrop().into(rbt);
    ...
    public void go()
    {
        ...
        if (inputMethodManager.isActive())
        {
            try
            {
                inputMethodManager.hideSoftInputFromWindow(ImagePagerActivity.this.getCurrentFocus().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
            }
