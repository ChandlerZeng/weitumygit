...
public class CdFragment extends NotifyFragment
{
        ...
        listview.setOnItemClickListener(new OnItemClickListener()
        {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position, long id)
            {
                ...
                else if ("image".equals(dto.type))
                {
                    ...
                    intent.putExtra("url", ContantsUtil.getFile(lib, fragment.getDto().mid, curDisc, dto.id));
                ...
                else if ("video".equals(dto.type))
                {
                    ...
                    bundle.putString(VideoPlayActivity.MEDIA_PATH, ContantsUtil.getFile(lib, fragment.getDto().mid, curDisc, dto.id));
    ...
    private List<MediaResult> conver(List<CdDto> source)
    {
        ...
        for (CdDto dto : source)
        {
            ...
            dat.title = dto.name;
            dat.url = ContantsUtil.getFile(lib, fragment.getDto().mid, curDisc, dto.id);
            result.add(dat);
    ...
    private void loadData(final long sequence)
    {
        ...
        OkHttpUtils.post().url(ContantsUtil.CD_DETAIL + "?lid=" + lib + "&bid=" + fragment.getDto().mid + "&sequnce=" + curDisc + "&id=" + sequence).build().execute(new StringCallback()
        {
            @Override
