 ...
 */
public class ClickHistoryAdapter extends BaseAdapter {
...
//    public DisplayImageOptions mOptions;
    ...
    public ClickHistoryAdapter(Context context, List<ResultBean> list) {
...
//                .cacheOnDisc(true).build();
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (typeF==ClickHistoryActivity.BOOK) {
        ...
        holder1.titleText.setText(mlist.get(position).target.title);
        holder1.uploaderText.setText("上传者:" + mlist.get(position).target.uploadUsername);
        holder1.introductionText.setText(mlist.get(position).target.introduction);
        //  checkView(holder1.checkMark, mlist.get(position).ischecked);
        holder1.checkMark.setChecked(allCheck);
        if (typeF==ClickHistoryActivity.DOC) {
            holder1.iconCover.setImageResource(DocType.getDocType(mlist.get(position).target.title));
        } else {
            String a = "http://cover1.bookday.cn/" + mlist.get(position).target.cover;
...
//                    mOptions, null);
        ...
        holder1.checkMark.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                mlist.get(position).target.ischecked = isChecked;
            }
        });
        return convertView;
    }
    ...
    public void setNewData(List<ResultBean> list) {
        this.mlist = list;
        notifyDataSetChanged();
    }
    ...
    public void setAllView() {
        ...
        for (int i = 0; i < mlist.size(); i++)
            mlist.get(i).target.ischecked = allCheck;
        notifyDataSetChanged();
    }
    ...
    public String[] cleanView() {
        ...
        for (int i = 0; i < mlist.size(); i++) {
            if (mlist.get(i).target.ischecked) {
                Log.e("" + mlist.get(i).target.ischecked);
